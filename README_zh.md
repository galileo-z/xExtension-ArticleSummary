...
- **多API提供商支持**：支持 OpenAI、Ollama、Gemini 和 LM Studio API 提供商，提供最大的灵活性。
...
- **自动 API 版本处理**：如果缺少 API 版本路径，会自动将适当的版本路径（例如 OpenAI 兼容提供商的 `/v1` 或 Gemini 的 `/v1beta`）添加到基础 URL。
...
1. **基础 URL**：输入您的语言模型 API 的基础 URL（例如，`https://api.openai.com/`）。通常不需要包含版本路径（例如，`/v1`），除非您想显式填写。LM Studio 的默认本地服务地址通常是 `http://localhost:1234`。
2. **API 密钥**：提供您的 API 密钥用于身份验证。Ollama 和 LM Studio 默认可以留空，除非您启用了身份验证。
...
- 版本: 0.6.0 (2026-05-25)
  > **新增功能**:
  > - 增加对 LM Studio 的支持，作为 OpenAI 兼容提供商使用
  > - 使用 LM Studio 的流式 Chat Completions 接口
  > - 默认允许 LM Studio 不填写 API 密钥
